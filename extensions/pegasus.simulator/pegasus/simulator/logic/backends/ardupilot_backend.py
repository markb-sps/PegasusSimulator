"""
| File: ardupilot_backend.py
| Author: Mark Bentley
| Description: File that implements the backend for communication/control with/of the vehicle simulation
"""
...
class SensorMsg:
    """
    An auxiliary data class where we write all the sensor data that is going to be sent to the sitl FC.
    """
...
class ThrusterControl:
    ...
    def __init__(
        ...
        # Values to scale and offset the rotor control inputs received from SITL
...
class ArdupilotBackendConfig:
    ...
    def __init__(self, config={}):
        ...
        Examples:
            ...
            connection_sensor_port
            connection_control_port

            >>>  "sitl_autolaunch": True,
            >>>  "sitl_dir": "PegasusInterface().sitl_path",
            >>>  "sitl_vehicle_model": "iris",
        ...
        self._sensor_port = config.get("connection_sensor_port", 5511)
        self._control_port = config.get("connection_control_port", 5501)
        ...
        # Configure whether to launch sitl in the background automatically or not for every vehicle launched
        self.sitl_autolaunch: bool = config.get("sitl_autolaunch", True)
        self.sitl_dir: str = config.get("sitl_dir", "")
        self.sitl_vehicle_model: str = config.get("sitl_vehicle_model", "iris")
        ...
        # The update rate at which we will be sending data to the FC (TODO - remove this from here in the future
        # and infer directly from the function calls)
        self.update_rate: float = config.get("update_rate", 250.0)  # [Hz]


class ArdupilotBackend(Backend):
    """ The Ardupilot Backend used to receive the vehicle's state and sensor data in order to send to the Ardu SITL FC over UDP. It also
    receives via UDP the thruster commands to apply to each vehicle rotor.
    """

    def __init__(self, config=ArdupilotBackendConfig()):
        """Initialize the MavlinkBackend

        Args:
            config (ArdupilotBackendConfig): The configuration class for the ArdupilotBackend.
        """
        ...
        self._connection_port = (config.connection_ip, config._control_port + config.vehicle_id)
        self._sensor_port = (config.connection_ip, config._sensor_port + config.vehicle_id)

        self._rx_comms_thread = None
        self._rx_comms_thread_should_stop = threading.Event()

        # Check if we need to autolaunch sitl in the background or not
        self.sitl_autolaunch: bool = config.sitl_autolaunch
        self.sitl_vehicle_model: str = config.sitl_vehicle_model  # only needed if sitl_autolaunch == True
        self.sitl_tool = None
        self.sitl_dir: str = config.sitl_dir
    ...
    def update_gps_data(self, data):
        ...
        self._sensor_data.latitude_deg = data["latitude"]
        self._sensor_data.longitude_deg = data["longitude"]
        self._sensor_data.altitude = data["altitude"]
        self._sensor_data.eph = int(data["eph"])
        self._sensor_data.epv = int(data["epv"])
        self._sensor_data.velocity = data["speed"]
        self._sensor_data.velocity_north = data["velocity_north"]
        self._sensor_data.velocity_east = data["velocity_east"]
        self._sensor_data.velocity_down = data["velocity_down"]
        self._sensor_data.cog = data["cog"]
    ...
    def stop(self):
        ...
        if self.sitl_autolaunch and self.sitl_autolaunch is not None:
            carb.log_info("Attempting to kill SITL background process")
            self.sitl_tool.kill_sitl()
            self.sitl_tool = None
    ...
    def _rx_comms_thread_func(self):
        while not self._rx_comms_thread_should_stop.is_set():
            try:
                buffer_size = 1024
                data = self._control_connection.recv(buffer_size)
                print("============")

                actuator_values = array.array('H',data)

                print(actuator_values)
                self._rotor_data.update_input_reference(actuator_values)
                ...
                # This exception is caused by the client being shut down.
                raise
    ...
    def re_initialize_interface(self):
        ...
        # Create the control and sensor info sockets.
        self._control_connection = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self._control_connection.bind(self._connection_port)

        self._rx_comms_thread_should_stop.clear()
        self._rx_comms_thread = threading.Thread(target=self._rx_comms_thread_func,
                                       name="rx_comms_thread")
        self._rx_comms_thread.start()

        self._sensor_data_connection = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        # self._sensor_data_connection.bind(self._sensor_port)
    ...
    def update(self, dt):
        ...
        # TODO: self.poll_mavlink_messages()

        # Send hearbeats at 1Hz TODO: from offboard mode code not sim code
    ...
    def send_heartbeat(self, mav_type):
        """
    ...
    def send_sensor_msgs(self, time_usec: int):
        ...
        try:
            ...
            rot = Rotation.from_quat(self._sensor_data.sim_attitude)
            euler_pose = rot.as_euler('zyx', degrees=False)

            if np.isnan(self._sensor_data.xgyro):
                return
            data = {"timestamp": time_usec,
                    "imu": {"angular_velocity": [self._sensor_data.xgyro, self._sensor_data.ygyro, self._sensor_data.zgyro],
                    "linear_acceleration": [self._sensor_data.xacc, self._sensor_data.yacc, self._sensor_data.zacc]},
                    "pose": {"pitch": euler_pose[0], "roll": euler_pose[1], "yaw": euler_pose[2]},
                    "gps": {"lat": self._sensor_data.latitude_deg, "lon": self._sensor_data.longitude_deg, "alt": self._sensor_data.altitude},
                    "velocity": {"world_linear_velocity": [self._sensor_data.velocity_north, self._sensor_data.velocity_east, self._sensor_data.velocity_down]}
                    }
            print(f"sending: {data}")
            data_json = json.dumps(data) + '\n'
            self._sensor_data_connection.sendto(bytes(data_json, 'utf-8'), self._sensor_port)
            #     time_usec,
